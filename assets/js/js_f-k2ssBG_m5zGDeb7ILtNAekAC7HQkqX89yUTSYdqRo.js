...
(function ($) {
        ...
        .each(function() {
          ...
          if (fieldName in settings.shs) {
            ...
            $.each(fieldSettings.parents, function(index, parent) {
              ...
              getTermChildren($select, fieldSettings, parent_id, parent.tid, $field.attr('id'), level);
            ...
            if (addNextLevel) {
              ...
              $select.appendTo($field.parent());
              // Retrieve data for this level.
              getTermChildren($select, fieldSettings, parent_id, 0, $field.attr('id'), level);
            }
          }
        });
    }
   ...
   * @param parent_value
    ...
    */
  getTermChildren = function($element, settings, parent_value, default_value, base_id, level) {
      ...
      success: function(data) {
        if (data.success == true) {
          ...
          if (!settings.settings.required || (settings.settings.required && parent_value != 0 && !settings.multiple)) {
            if(level == 1) {
              options[options.length] = new Option(Drupal.t('Страна'), 0);
            }
            else if(level == 2) {
              options[options.length] = new Option(Drupal.t('Регион'), 0);
            }
          }
  ...
  updateElements = function($triggering_element, base_id, settings, level) {
    ...
    if ($triggering_element.val() == '_add_new_') {
      ...
      getTermChildren($element_new, settings, $triggering_element.val(), 0, base_id, level);
    }
